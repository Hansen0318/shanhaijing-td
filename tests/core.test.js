...
test('configuration contains the six-beast catalog and preserves level-one waves', () => {
  assert.equal(GAME_CONFIG.initialGold, 300);
  assert.equal(GAME_CONFIG.baseHp, 20);
  assert.deepEqual(Object.keys(TOWER_DATA), ['bifang', 'fuzhu', 'yinglong', 'baize', 'jumang', 'xuangui']);
  ...
  assert.equal(BLESSING_DATA.length, 18);
