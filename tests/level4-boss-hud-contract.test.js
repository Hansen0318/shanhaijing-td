...
test('all seven Boss HUDs use one variable-positioned code-driven fill without image masks', async () => {
  ...
  assert.match(art, /zhulongBossPanel:\s*'assets\/ui\/ui_boss_zhulong_panel_v2\.png'/);
  ...
  assert.match(base, /\.boss-hud\[data-boss-type="zhulong"\] strong\s*\{[^}]*top:\s*7px[^}]*left:\s*39\.4%[^}]*width:\s*21\.2%/);
  ...
  assert.match(html, /styles\.css\?v=level9-visual-3/);
  ...
  assert.match(ui, /artAssets\.js\?v=asset-load-3/);
  assert.match(renderer, /artAssets\.js\?v=level9-visual-3/);
